...
# Conecta la señal 'animation_finished' de tu nodo BodyAnimations a esta función
# (puedes hacerlo desde el panel de Nodos a la derecha en Godot, o por código en _ready)
func _on_body_animations_animation_finished(anim_name: StringName) -> void:
	# Debes enviarle este aviso a la máquina de estados. 
	# Asumiendo que tu StateMachine es un nodo hijo llamado "StateMachine":
