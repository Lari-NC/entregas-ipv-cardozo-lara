...
@export var pathfinding: PathfindAstar = get_node_or_null(pathfinding_path)
...
func _physics_process(_delta: float) -> void:
	...
	if !path.is_empty():
		...
		# Verificamos de nuevo que no esté vacío antes de movernos
		if !path.is_empty(): 
			var asa = velocity + global_position.direction_to(next_point) * speed
			velocity = asa.limit_length(max_speed)
			
			# Damos vuelta el sprite al caminar hacia el punto
			body_anim.flip_h = velocity.x > 0
	else:
		# Solo miramos al target si estamos quietos y tenemos un objetivo
		if target != null:
